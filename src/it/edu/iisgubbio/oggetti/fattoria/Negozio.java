...
public class Negozio extends Application{
	...
	private void eseguiMuffato() {
		...
		muffato.setTempoDiStagionatura(Integer.parseInt(tfStagionatura.getText()));
		muffato.setInterno(cbInterna.isSelected());
		muffato.setDop(tbDOP.isSelected());
		lvProdotti.getItems().add(muffato);
	}
	
	private void eseguiCarne() {
		
		Carne carne = new Carne(tfNome.getText(),Double.parseDouble(tfPrezzo.getText()),cbKosher.isSelected(),cbCarneBianca.isSelected());
